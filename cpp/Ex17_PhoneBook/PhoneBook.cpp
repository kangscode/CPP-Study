...
void PhoneBook::AddContact()
{
    ...
    Contact* new_contacts_ = new Contact[num_contacts_ + 1];
    for (int i = 0; i < num_contacts_; i++) {
        memcpy(new_contacts_[i].name, contacts_[i].name, sizeof(contacts_[i].name));
        memcpy(new_contacts_[i].name, contacts_[i].name, sizeof(contacts_[i].name));
    }

    delete[] contacts_;
    contacts_ = new_contacts_;
...
void PhoneBook::DeleteByName()
{
    ...
    if (index >= 0)
    {
        ...
        for (int i = index; i < num_contacts_ - 1; i++) {
