...
public:
	...
	void Resize() // 2배씩 증가
	{
		...
		if (rear_ == capacity_ - 1)
			memcpy(new_queue_ + 1, queue_ + front_ + 1, sizeof(T) * capacity_);
		else
		{
			memcpy(new_queue_+ 1, queue_ + front_ + 1, sizeof(T) * (capacity_ - 1 - front_));
			memcpy(new_queue_ + (capacity_ - front_), queue_, sizeof(T) * (rear_ + 1));
		}
		delete[] queue_;
		queue_ = new_queue_;
