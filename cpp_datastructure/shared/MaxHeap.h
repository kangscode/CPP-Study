...
public:
	...
	void Pop()
	{
		...
		while (child <= size_)
		{
			...
			// TODO:
			int max_child = child; // 일단 왼쪽이 크다고 가정
			if (child + 1 <= size_ && heap_[child] < heap_[child + 1]) {
				max_child = child + 1; // 오른쪽이 존재하고 더 크다면 오른쪽 선택
			}

			// 마지막 값이 더 큰 자식의 값 이상이면 더이상 적절한 위치를 찾을 필요가 없기 때문에 루프 중단
			// TODO:
			if (last_item >= heap_[max_child])
				break;

			// 자식 값을 부모 위치로 복사, 
			// TODO:
			heap_[current] = heap_[max_child];
